...
function PeriodClose() {
  ...
  async function close(e: FormEvent) {
    ...
    try {
      ...
      toast.success(`Books closed — net ${r.netProfit} rolled to retained earnings (${r.entryNumber})`);
      setThrough("");
  ...
  return (
    <>
      <PageHeader title="Period Close" subtitle="Freeze a settled period and roll profit into retained earnings" />
      <div className="p-6">
        <Card className="mx-auto max-w-xl p-6">
          <div className="mb-6 flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-widest text-muted-foreground">Books closed through</p>
              <p className="mt-1 text-2xl font-bold">{status.closedThrough ?? "Open — never closed"}</p>
            </div>
            <Badge
              variant="outline"
              className={status.closedThrough ? "border-warning/30 bg-warning/10 text-warning-foreground" : "border-success/20 bg-success/10 text-success"}
            >
              {status.closedThrough ? <><Lock className="mr-1 h-3 w-3" />Locked</> : <><LockOpen className="mr-1 h-3 w-3" />Open</>}
            </Badge>
          </div>
          ...
          {canClose ? (
            <div className="space-y-6">
              <form onSubmit={close} className="space-y-3 border-t pt-5">
                ...
                <div className="flex gap-2">
                  <Input id="pc-through" type="date" value={through} onChange={(e) => setThrough(e.target.value)} />
                ...
                <p className="text-xs text-muted-foreground">
                  Zeroes every income and expense account into retained earnings and blocks any posting on or before this date.
                </p>
              </form>
              ...
              {status.closedThrough ? (
                <form onSubmit={reopen} className="space-y-3 border-t pt-5">
                  ...
                  <div className="flex gap-2">
                    <Input id="pc-reason" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Why are you reopening?" />
            ...
            <p className="border-t pt-5 text-sm text-muted-foreground">Closing the books requires the admin role.</p>
          )}
