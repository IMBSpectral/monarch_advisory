...
function GRN() {
  ...
  async function bill(id: string) {
    setBusy(id);
    try {
      const r = await convertGrnToBillFn({ data: { goodsReceiptId: id, billDate: new Date().toISOString().slice(0, 10) } });
              ...
              {receipts.length === 0 ? (
                ...
                receipts.map((g) => (
                  <TableRow key={g.id} className="hover:bg-muted/40">
                    ...
                    <TableCell className="text-right font-medium tabular-nums">{formatMinor(g.total)}</TableCell>
                    <TableCell>
                      <Badge variant="outline" className={g.billId ? noteStatusStyle.invoiced : noteStatusStyle[g.status]}>
                        {g.billId ? "Billed" : noteStatusLabel[g.status] ?? g.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      {canPost && g.status === "posted" && !g.billId ? (
                        <Button size="sm" variant="outline" disabled={busy === g.id} onClick={() => bill(g.id)}>
                          {busy === g.id ? <Loader2 className="h-4 w-4 animate-spin" /> : "Raise bill"}
                        </Button>
                      ) : (
...
function NewGrn({ vendors, items }: { vendors: any[]; items: any[] }) {
  ...
  async function submit(e: FormEvent) {
    ...
    setPending(true);
    try {
      const r = await createGoodsReceiptFn({ data: { contactId, receiptDate: date, lines: usable } });
      setOpen(false); reset();
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <form onSubmit={submit}>
          <DialogHeader>
            ...
            <DialogDescription>Receive stock now; the bill can follow. Posts Dr Inventory / Cr GRNI.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                ...
                <Input id="grn-date" type="date" value={date} onChange={(e) => setDate(e.target.value)} required />
              </div>
            </div>
            <LinesEditor lines={lines} setLines={setLines} items={items} taxRates={[]} priceField="purchasePrice" showTax={false} requireItem />
          </div>
          {error ? <p role="alert" className="mb-2 text-sm text-destructive">{error}</p> : null}
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={pending}>Cancel</Button>
            <Button type="submit" disabled={pending}>
              {pending ? <><Loader2 className="mr-2 size-4 animate-spin" />Posting…</> : "Receive stock"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
