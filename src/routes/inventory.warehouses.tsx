          ...
          <Card key={w.name} className="p-5">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-brand"><Warehouse className="h-5 w-5 text-white" /></div>
                <div>
                  <h3 className="font-semibold">{w.name}</h3>
                  <p className="text-xs text-muted-foreground flex items-center gap-1"><MapPin className="h-3 w-3" />India</p>
                </div>
              </div>
              <Badge variant="secondary">{w.util}% used</Badge>
            </div>
            <div className="grid grid-cols-3 gap-4 mt-5">
              <div><p className="text-xs text-muted-foreground">Zones</p><p className="text-lg font-semibold tabular-nums">{w.zones}</p></div>
              <div><p className="text-xs text-muted-foreground">Bins</p><p className="text-lg font-semibold tabular-nums">{w.bins}</p></div>
              <div><p className="text-xs text-muted-foreground">Items</p><p className="text-lg font-semibold tabular-nums">{w.items}</p></div>
            </div>
