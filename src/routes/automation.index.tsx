...
const flows = [
  ...
  {
    name: "Large Bill Approval",
    trigger: "Bill amount > ₹1L",
    action: "Route to CFO approval",
    runs: 42,
    active: true,
  },
  {
    name: "Low Stock Purchase",
...
const script: Omit<LogEntry, "ts">[] = [
  {
    level: "info",
    msg: "Workflow invoked",
    detail: "Overdue Invoice Reminder · manual simulation",
  },
  { level: "trigger", msg: "TRIGGER fired", detail: "cron:hourly · scanning 184 open invoices" },
  {
    level: "info",
    msg: "Query executed",
    detail: "SELECT * FROM invoices WHERE status='overdue' — 27 rows in 42ms",
  },
  {
    level: "condition",
    msg: "CONDITION evaluated",
    detail: "days_overdue > 3 AND balance > ₹10,000 → 9 matching invoices",
  },
  { level: "info", msg: "Loading customer contacts", detail: "9 recipients resolved from CRM" },
  {
    level: "action",
    msg: "ACTION dispatched",
    detail: "template=polite_reminder_v2 · channel=email + slack",
  },
  {
    level: "info",
    msg: "Rendering templates",
    detail: "9 personalised messages · GST-compliant footer",
  },
  { level: "success", msg: "8 emails delivered", detail: "SendGrid 202 · avg latency 312ms" },
  {
    level: "warn",
    msg: "1 delivery deferred",
    detail: "invoice INV-2087 · retry scheduled in 15m",
  },
  {
    level: "action",
    msg: "Escalation queued",
    detail: "INV-2039 unpaid 7+ days → Account Manager notified",
  },
  {
    level: "success",
    msg: "Workflow completed",
    detail: "9 processed · 8 succeeded · 1 retry · total 1.42s",
  },
];
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Visual builder for approvals, notifications & escalations"
        actions={
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={() =>
                toast.info(
                  "Workflow builder coming soon — automations run server-side in production.",
                )
              }
            >
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Auto-generated draft · last edited 2h ago
              </p>
            </div>
                ...
                <Fragment key={s.label}>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`text-xs uppercase tracking-widest font-medium ${toneText}`}>
                        {s.label}
                      </span>
                      {active && !current && (
                      ...
                      {current && (
                        <Loader2 className="h-3.5 w-3.5 text-brand ml-auto animate-spin" />
                      )}
                    </div>
        ...
        <Card className="overflow-hidden border-primary/20">
          <div className="flex items-center justify-between bg-primary text-primary-foreground px-4 py-2.5">
            ...
            <div className="flex items-center gap-1.5 text-[11px] font-mono opacity-80">
              <span
                className={`h-2 w-2 rounded-full ${running ? "bg-emerald-400 animate-pulse" : "bg-muted-foreground/50"}`}
              />
          ...
          >
            {logs.length === 0 && !running && (
              <div className="text-slate-500 italic">
                Press <span className="text-brand">Simulate Run</span> to execute this workflow and
                stream trigger → condition → action logs here.
              </div>
            )}
            {logs.map((l, i) => (
              <div
              ...
              >
                ...
                <span
                  className={`uppercase tracking-wider text-[10px] font-semibold shrink-0 w-20 ${levelStyles[l.level]}`}
                >
                  {l.level}
                </span>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="mt-2 text-xs text-muted-foreground space-y-0.5">
                      <p>
                        <span className="text-brand">When</span> {f.trigger}
                      </p>
                      <p>
                        <span className="text-brand">Do</span> {f.action}
                      </p>
                    </div>
                  </div>
