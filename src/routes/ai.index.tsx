...
import { Sparkles, Send, Brain, TrendingUp, AlertTriangle, FileCheck, MessageSquare, Cpu } from "lucide-react";
      ...
      <div className="p-6 grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-4">
          ...
          <Card className="p-0 overflow-hidden">
            <div className="p-4 border-b bg-gradient-to-r from-brand/5 to-transparent flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">GPT-4.1 · Fine-tuned on Indian accounting</p>
              </div>
            ...
            <div className="p-4 space-y-4 min-h-[420px]">
              <div>
                <div className="max-w-[80%] rounded-2xl rounded-tl-sm bg-muted px-4 py-3 text-sm">
                  Hi Arjun 👋 I've reviewed today's activity. Ask me anything about your books, cash position, or vendors.
                </div>
              </div>
              ...
              <div>
                <div className="max-w-[80%] rounded-2xl rounded-tl-sm bg-muted px-4 py-3 text-sm space-y-2">
                  <p>You have <span className="font-semibold">2 overdue invoices</span> totaling <span className="font-semibold">₹2,57,400</span>:</p>
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button size="sm" className="bg-gradient-brand text-white h-7 text-xs">Send Reminders</Button>
              ...
              <div>
                <div className="max-w-[95%] rounded-2xl rounded-tl-sm bg-muted p-4">
                  <p className="text-sm mb-3">Based on trailing 90-day trends and confirmed pipeline, here's your cash forecast:</p>
                  <ResponsiveContainer width="100%" height={160}>
                    <AreaChart data={cashForecast}>
                      ...
                      <Area type="monotone" dataKey="forecast" stroke="oklch(0.55 0.14 165)" strokeWidth={2} fill="url(#ai-g)" />
                    </AreaChart>
                  </ResponsiveContainer>
                  <p className="text-xs text-muted-foreground mt-2">Projected to reach <span className="font-semibold text-brand">₹83L</span> by July. Confidence 87%.</p>
                </div>
              </div>
            </div>
            <div className="border-t p-3 flex gap-2 bg-background">
              <Input placeholder="Ask about your business…" className="bg-muted/40" />
              <Button className="bg-gradient-brand text-white"><Send className="h-4 w-4" /></Button>
            </div>
          </Card>
        </div>
        ...
        <div className="space-y-4">
          <Card className="p-5">
            ...
            {[
              { i: FileCheck, n: "AI Accountant", d: "Auto-categorizes 94% of transactions", s: "18 pending review" },
              { i: TrendingUp, n: "AI CFO", d: "Weekly digest + variance alerts", s: "Next run: Monday 9 AM" },
              { i: MessageSquare, n: "Chat Assistant", d: "Natural language ERP queries", s: "412 queries this week" },
              { i: AlertTriangle, n: "Anomaly Detector", d: "Flags unusual spend patterns", s: "2 alerts today" },
            ].map((a) => (
              <div key={a.n} className="flex items-start gap-3 py-3 border-b last:border-0">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-brand"><a.i className="h-4 w-4 text-white" /></div>
          ...
          <Card className="p-5">
            ...
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-xs mb-1"><span>Queries</span><span className="tabular-nums">1,842 / 10,000</span></div>
                <div className="h-1.5 rounded-full bg-muted overflow-hidden"><div className="h-full bg-gradient-brand" style={{ width: "18%" }} /></div>
              </div>
              <div>
                <div className="flex justify-between text-xs mb-1"><span>Tokens</span><span className="tabular-nums">2.4M / 20M</span></div>
                <div className="h-1.5 rounded-full bg-muted overflow-hidden"><div className="h-full bg-gradient-brand" style={{ width: "12%" }} /></div>
              </div>
              <div>
                <div className="flex justify-between text-xs mb-1"><span>Automations</span><span className="tabular-nums">306 runs</span></div>
                <div className="h-1.5 rounded-full bg-muted overflow-hidden"><div className="h-full bg-gradient-gold" style={{ width: "34%" }} /></div>
              </div>
            </div>
          </Card>
        </div>
